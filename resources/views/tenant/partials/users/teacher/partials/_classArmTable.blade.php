                                    ...
                                    <td class="max-w-0  px-6 py-4 whitespace-nowrap text-xs text-gray-900">
                                        <div class="flex">
                                            <p class="group inline-flex space-x-2 truncate">
                                                ...
                                                <span class="text-gray-500 truncate capitalize" x-text="content.class_section ? content.class_section.section_name : ''"></span>
                                                <span class="text-gray-500 truncate capitalize" x-text="content.class_section_category ? content.class_section_category.category_name : ''"></span>
                                            </p>
                                        </div>
                                    </td>
...
<script>
    function classArmTable() {
        return{
            search: "",
            pageNumber: 0,
            size: 5,
            total: "",
            classArmData: {!! $classArms !!},
            get filteredClassArmTable() {
                const start = this.pageNumber * this.size,
                    end = start + this.size;
                if (this.search === "") {
                    this.total = this.classArmData.length;
                    return this.classArmData.slice(start, end);
                }
                //Return the total results of the filters
                this.total = this.classArmData.filter((item) => {
                    return item.subject_name
                        .toLowerCase()
                        .includes(this.search.toLowerCase());
                }).length;
                //Return the filtered data
                return this.classArmData
                    .filter((item) => {
                        return item.subject_name
                            .toLowerCase()
                            .includes(this.search.toLowerCase());
