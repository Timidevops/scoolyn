<div class="lg:px-8">
    <div>
        ...
        <a href="{{route('listStudentFee',$student->uuid)}}"><span class="mt-2  text-sm text-gray-300">/!/ Back to school fees</span></a>
    </div>
    <div class="bg-white rounded-md mt-8" x-data="schoolFeesReceipt()">
        <div class="mt-8">
            <div class=" sm:block">
                    ...
                    <div class="flex flex-col mt-2">
                        <div class="align-middle min-w-full overflow-x-auto  overflow-hidden ">
                            <table class="min-w-full divide-y  divide-purple-100">
                                ...
                                <template x-for="(content, index) in filteredSchoolFeesReceiptTable" :key="index">
                                    <tr class="bg-white">
                                        ...
                                        <td class="max-w-0  px-6 py-4 whitespace-nowrap text-xs text-gray-900">
                                            <div class="flex">
                                                <p class="group inline-flex space-x-2 truncate">
                                                    <span class="text-gray-500 truncate capitalize" x-text="content.amount"></span>
                                                </p>
                                            </div>
                                        </td>
                                        ...
                                        <td class="max-w-0  px-6 py-4 whitespace-nowrap text-xs text-gray-900">
                                            <div class="flex">
                                                <p class="group inline-flex space-x-2 truncate">
                                                    <span class="text-gray-500 truncate capitalize" x-text="content.description ?? '...'"></span>
                                                </p>
                                            </div>
                                        </td>
...
<script>
    function schoolFeesReceipt() {
        return{
            search: "",
            pageNumber: 0,
            size: 10,
            total: "",
            schoolFeesReceiptData: {!! $transactions !!},
            get filteredSchoolFeesReceiptTable() {
                const start = this.pageNumber * this.size,
                    end = start + this.size;
                if (this.search === "") {
                    this.total = this.schoolFeesReceiptData.length;
                    return this.schoolFeesReceiptData.slice(start, end);
                }
                //Return the total results of the filters
                this.total = this.schoolFeesReceiptData.filter((item) => {
                    return item.subject_name
                        .toLowerCase()
                        .includes(this.search.toLowerCase());
                }).length;
                //Return the filtered data
                return this.schoolFeesReceiptData
                    .filter((item) => {
                        return item.subject_name
                            .toLowerCase()
                            .includes(this.search.toLowerCase());
