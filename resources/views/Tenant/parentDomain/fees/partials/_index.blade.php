...
<div class="bg-white rounded-md" x-data="fees()">
    ...
    @include('Tenant.parentDomain.fees.partials._indexTable')
</div>
...
<script>
    function fees() {
        return{
            ...
            size: 5,
            total: "",
            schoolFeesTable: {!! $schoolFees !!},
            get filteredSchoolFeesTable() {
                const start = this.pageNumber * this.size,
                    end = start + this.size;
                if (this.search === "") {
                    this.total = this.schoolFeesTable.length;
                    return this.schoolFeesTable.slice(start, end);
                }
                //Return the total results of the filters
                this.total = this.schoolFeesTable.filter((item) => {
                    return item.subject_name
                        .toLowerCase()
                        .includes(this.search.toLowerCase());
                }).length;
                //Return the filtered data
                return this.schoolFeesTable
                    .filter((item) => {
                        return item.subject_name
                            .toLowerCase()
                            .includes(this.search.toLowerCase());
