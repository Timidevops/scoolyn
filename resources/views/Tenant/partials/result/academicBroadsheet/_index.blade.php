<div>
    ...
    <span class="mt-2 text-base text-gray-300">{{$totalSubject}} Total Subjects</span>
</div>

<div class="bg-white rounded-md" x-data="academicBroadsheet()">
...
<script>
    function academicBroadsheet(){
        return{
            getTableClassSection(classSection, classSectionCategory, href = false){
                if( classSection && classSectionCategory ) {
                    return href ? `?class-section-category=${classSectionCategory.laravel_through_key}` : `${classSection.section_name} - ${classSectionCategory.category_name}`;
                }
                else if(classSection && ! classSectionCategory){
                    return href ? `?class-section=${classSection.laravel_through_key}` : classSection.section_name;
                }
                return href ? '?class-section=all-section' : 'All Sections';
            },
            search: "",
            pageNumber: 0,
            size: 5,
            ...
            get filteredSubjectTable() {
                ...
                this.total = this.subjects.filter((item) => {
