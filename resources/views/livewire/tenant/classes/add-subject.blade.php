<div class="mx-2 md:w-1/4">
    ...
    <form wire:submit.prevent="store" class="overflow-auto @if($isOpenAddSubjectModal) absolute inset-0 z-10 flex items-center justify-center @else hidden @endif" style="background-color:rgba(190,192,201,0.7);">
        @csrf
            ...
            <div class=" mx-4" >
              ...
              rounded-md shadow-xs max-h-60 focus:outline-none sm:text-sm sm:leading-5">
                ...
                <div class="my-6">
                    ...
                    <button wire:click="$set('subjectDropdown', {{!$subjectDropdown}})" type="button"
                            class="cursor-pointer z-0 w-full py-2 pl-3 pr-10 text-left text-gray-100 font-normal border border-purple-100 rounded-md cursor-default focus:outline-none focus:shadow-outline-blue focus:border-blue-300 sm:text-sm sm:leading-5">
                        {{$subjectLabel}}
                    </button>
                    <div class="border border-purple-100 @if(!$subjectDropdown)  hidden @endif">
                        <ul class="py-1 overflow-auto h-32 text-base leading-6 border border-purple-100
              rounded-md shadow-xs max-h-60 focus:outline-none sm:text-sm sm:leading-5">
                            @foreach($subjects as $subject)
                                <li wire:click="selectSubject('{{$subject->uuid}}', '{{$subject->subject_name}}')" class="relative py-2 pl-3  text-gray-200 cursor-pointer select-none pr-9">
